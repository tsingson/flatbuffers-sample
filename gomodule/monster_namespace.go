// monster example support namespace / go module
package main
...
func main() {
	...
	Example.WeaponStart(builder)
	Example.WeaponAddName(builder, weaponOne)
	Example.WeaponAddDamage(builder, 3)
	sword := Example.WeaponEnd(builder)

	Example.WeaponStart(builder)
	Example.WeaponAddName(builder, weaponTwo)
	Example.WeaponAddDamage(builder, 5)
	axe := Example.WeaponEnd(builder)

	// Serialize the FlatBuffer data.
	name := builder.CreateString("Orc")

	Mygame.MonsterStartInventoryVector(builder, 10)
	...
	Mygame.MonsterStartWeaponsVector(builder, 2)
	// Note: Since we prepend the weapons, prepend in reverse order.
	builder.PrependUOffsetT(axe)
	builder.PrependUOffsetT(sword)
	weapons := builder.EndVector(2)

	pos := Example.CreateVec3(builder, 1.0, 2.0, 3.0)

	Mygame.MonsterStart(builder)
	Mygame.MonsterAddPos(builder, pos)
	Mygame.MonsterAddHp(builder, 300)
	Mygame.MonsterAddName(builder, name)
	Mygame.MonsterAddInventory(builder, inv)
	Mygame.MonsterAddColor(builder, Example.ColorRed)
	Mygame.MonsterAddWeapons(builder, weapons)
	Mygame.MonsterAddEquippedType(builder, Example.EquipmentWeapon)
	Mygame.MonsterAddEquipped(builder, axe)
	orc := Mygame.MonsterEnd(builder)
	...
	// backwards.
	monster := Mygame.GetRootAsMonster(buf, 0)
	...
	assert(string(monster.Name()) == "Orc", "`string(monster.Name())`", string(monster.Name()),
		"\"Orc\"")
	assert(monster.Color() == Example.ColorRed, "`monster.Color()`",
		strconv.Itoa(int(monster.Color())), strconv.Itoa(int(Example.ColorRed)))
	...
	weapon := new(Example.Weapon) // We need a `Mygame.Weapon` to pass into `monster.Weapons()`
	...
	// data itself.
	assert(monster.EquippedType() == Example.EquipmentWeapon, "`monster.EquippedType()`",
		strconv.Itoa(int(monster.EquippedType())), strconv.Itoa(int(Example.EquipmentWeapon)))
	...
	if monster.Equipped(unionTable) {
		...
		if monster.EquippedType() == Example.EquipmentWeapon {
			unionWeapon := new(Example.Weapon)
